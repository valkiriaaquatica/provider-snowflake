...
type OnExternalTableInitParameters struct {
...
type OnExternalTableObservation struct {
...
type OnExternalTableParameters struct {
...
// OnExternalTableSpec defines the desired state of OnExternalTable
type OnExternalTableSpec struct {
	v1.ResourceSpec `json:",inline"`
	ForProvider     OnExternalTableParameters `json:"forProvider"`
	...
	// autoscaler.
	InitProvider OnExternalTableInitParameters `json:"initProvider,omitempty"`
}

// OnExternalTableStatus defines the observed state of OnExternalTable.
type OnExternalTableStatus struct {
	v1.ResourceStatus `json:",inline"`
	AtProvider        OnExternalTableObservation `json:"atProvider,omitempty"`
}
...
// OnExternalTable is the Schema for the OnExternalTables API. Resource used to manage streams on external tables. For more information, check stream documentation https://docs.snowflake.com/en/sql-reference/sql/create-stream.
...
type OnExternalTable struct {
	...
	// +kubebuilder:validation:XValidation:rule="!('*' in self.managementPolicies || 'Create' in self.managementPolicies || 'Update' in self.managementPolicies) || has(self.forProvider.externalTable) || (has(self.initProvider) && has(self.initProvider.externalTable))",message="spec.forProvider.externalTable is a required parameter"
	...
	Spec   OnExternalTableSpec   `json:"spec"`
	Status OnExternalTableStatus `json:"status,omitempty"`
}

// +kubebuilder:object:root=true

// OnExternalTableList contains a list of OnExternalTables
type OnExternalTableList struct {
	metav1.TypeMeta `json:",inline"`
	metav1.ListMeta `json:"metadata,omitempty"`
	Items           []OnExternalTable `json:"items"`
}

// Repository type metadata.
var (
	OnExternalTable_Kind             = "OnExternalTable"
	OnExternalTable_GroupKind        = schema.GroupKind{Group: CRDGroup, Kind: OnExternalTable_Kind}.String()
	OnExternalTable_KindAPIVersion   = OnExternalTable_Kind + "." + CRDGroupVersion.String()
	OnExternalTable_GroupVersionKind = CRDGroupVersion.WithKind(OnExternalTable_Kind)
)

func init() {
	SchemeBuilder.Register(&OnExternalTable{}, &OnExternalTableList{})
}
